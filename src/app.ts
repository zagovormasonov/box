...
export class TestApp {
  ...
  private async initiateTinkoffPayment(paymentData: PaymentData): Promise<void> {
    try {
      ...
      // Для СБП используем разные варианты параметров
      if (paymentData.paymentMethod === 'sbp') {
        // Пробуем разные комбинации параметров для СБП
        tinkoffPaymentData.DATA = {
          'sbp': 'true',
          'sbp_enabled': 'true',
          'qr': 'true'
        }
        console.log('Настройка оплаты через СБП с DATA параметрами')
      }
...
Метод оплаты: ${paymentData.paymentMethod === 'sbp' ? 'СБП (должен показать QR-код)' : 'Банковская карта'}
